...
def test_append_preserves_existing_issue_and_increments_count(tmp_path: Path):
    p = tmp_path / "hil-issues.yaml"
    append_issue(p, HILIssue(
        id="HIL-001", artifact="a", stage="s", severity="major",
        doer_said="", checker_said="", attempts=3, status="open",
    ))
    append_issue(p, HILIssue(
        id="HIL-002", artifact="b", stage="s", severity="minor",
        doer_said="", checker_said="", attempts=3, status="open",
    ))
    doc = _load(p)
...
def test_unresolved_count_tracks_open_only(tmp_path: Path):
    ...
    append_issue(p, HILIssue(
        id="HIL-001", artifact="a", stage="s", severity="major",
        doer_said="", checker_said="", attempts=3, status="resolved",
    ))
    append_issue(p, HILIssue(
        id="HIL-002", artifact="b", stage="s", severity="major",
        doer_said="", checker_said="", attempts=3, status="open",
    ))
    doc = _load(p)
    assert doc["unresolved_count"] == 1
...
def test_hil_issue_with_suggested_fixes(tmp_path: Path):
    p = tmp_path / "hil-issues.yaml"
    append_issue(p, HILIssue(
        id="HIL-001", artifact="a.md", stage="mermaid", severity="major",
        doer_said="diagram", checker_said="hallucinated node",
